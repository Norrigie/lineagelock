...
Its ordering was taken from the game where the game states it — each gym map records which badge it awards — and confirmed against the community wiki where it doesn't. The resulting ladder climbs without a single dip, from Basil at Lv 12 to Gennaro at Lv 100, which is the check that the order is right rather than merely plausible. Leader names are the English build's: the game's data is Italian, and its own translation file supplies *Ginger*, *Vanilla*, *Nuphar*, *Caspar*, *Henna*, *Abraham* and the rest.
