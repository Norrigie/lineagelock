...
3. **Setup 3 — Choose your encounter rule: 👣 First Encounter, 🎬 Audition, 🎡 Spin the Wheel or 🌍 Catch 'em All.** Also locked for the run (Rule 15). First Encounter is the classic and works on anything. Audition still gives you one catch per area, but lets you choose which Pokémon there it is — choice without abundance. Catch 'em All lifts the per-area limit entirely and lets the die alone do the filtering — the friendly option for a first run. The wheel uses each area's real encounter table and appearance rates to name your target — bundled for 15 mainline games; if your game isn't one of them the tracker says so and you use First Encounter.
...
**🎬 Audition.** **One catch per area, and you choose which one.** Keep meeting wild Pokémon in the area until one of them is the Pokémon you want, and catch that one. The area is then spent, exactly as it would be under First Encounter — you don't get to come back for a second.
...
**Rule 29 — Level Cap (optional, chosen at setup).** No Pokémon may be **higher level than the next Major Battle's ace** — or than **one level above the cap you played the last wall under**, whichever is greater. **A cap never drops, and every wall you beat raises it by at least one.** Cross that line before the battle and it is over-levelled: it hands its chair over at the next PC and **its tour ends**, exactly as if the term had run out.
...
- **A cap only ever rises, and never by less than one.** Plenty of walls are weaker than the one before them — a rival after a gym, Decay's Jupiter sitting a level under Brawly, Ultra Sun's Hau a full 25 levels under Mina. Two things follow.
  ...
  Worked through, in Decay: Mandy's ace is 5, so the cap is 5. Courtney's is 13 → cap 13. Brawly's is 17 → cap 17. Jupiter's is **16**, which is lower, so the cap becomes **18** — one above Brawly's. The wall got easier; your ceiling still went up.

  A Rule 37 chain is **one** wall and gets **one** +1, not one per member.

  The honest consequence: in a region the designers levelled downward, the cap drifts above what you are actually fighting. Kanto in Gold · Silver is the extreme — six low gyms in a row put the cap 16 levels over Janine's ace. Everywhere else the drift stays inside 5, and in nine of the twenty-two bundled games it never appears at all. It also means the last wall's cap can sit a level or two above the Champion's own ace; if you would rather the cap be exactly what you face and never more, turn the dial off.
...
- **The tracker knows the numbers, and checks them with you.** Every major battle in all 21 mainline games and in both bundled fan games ships with its ace level. It asks about a wall only *after* you have beaten it — by then you have seen the team, so you can actually vouch for the number. It names the battle you just fought, shows the ace level it has on file, and asks once whether that is right. Say yes and it is locked in for the run; say no and your number overrides the bundled one permanently. Either way it never asks about that wall again. Because every later cap is built on that number, a correction here still matters: raising a wall you have already beaten raises the cap you play under next. A Rule 37 chain counts as one wall, so the Elite Four is confirmed once at the end, not five times. On a ROM hack with no bundled list it simply asks what that team’s highest level was.
...
`lineagelock-tracker.html` — open it in any browser, no install, no build, no account. Keep `decay-sprites.js` next to it if you want Pokémon Decay's own artwork; everything else works without it. Runs are saved in that browser, so bookmark the file and use the same browser all run — and **export a backup** now and then (below).
...
- **Only the routes you can still use** — under First Encounter, Audition or the Wheel, the location dropdown in the catch flow lists what is still **open**: an area you have already caught in, or burned, is not offered, and a footnote says how many were hidden. Whatever a Pokémon already records stays selectable, so editing one can never lose its place. Catch 'em All lists everything, because there an area is worth returning to until its table is complete (Rule 19).
- **What a wall specialises in** — where the game has a type speciality, it is shown as a **type badge** beside the battle's name, on the Next Major card and on every checklist row: Brock wears ROCK, Misty WATER, Sword · Shield's Gordie / Melony pair wears ROCK and ICE, and Black · White's trio gym wears all three. It is read out of the battle's own description, so a grand trial (FIGHTING), a Team Star base (DARK) and a titan (STEEL) all get one too, and a wall with no known speciality — a rival, a villain-team boss, a noble — simply shows none. Decay's walls never carried a type in the first place, so theirs were worked out from the teams the game gives them: **a leader's speciality is the type their whole team shares**, which puts a badge on all eight of its gyms and a handful of themed bosses. The bar is deliberately high — a looser majority rule would label Decay's Lance *Flying*, because his dragons happen to fly — so a genuinely mixed team gets nothing. There is one other way to earn a badge, and only one: **saying so**. Decay's Lance introduces himself at the Indigo Plateau as *"Lance the Dragon Trainer"*, and a declaration beats a tally, so he wears DRAGON although his team would not pass the team test. Every wall's pre-battle text was scanned for a claim like that and his is the only one, which is why the rest of Decay's Elite Four stay bare: Flint is three Fire out of five, Wallace three Water out of five, and Mr. Fuji has no theme at all. They are not specialists the way its gym leaders are.
- **Where each wall is** — every one of the **550 bundled majors** names the place it happens: the town, gym, cave or route, on the Next Major card and on every row of the checklist. Both fan games' locations were read out of their own map files — Decay's dev sheet records levels but not places, and Xenoverse's are in Italian in the data and needed the game's translation file to come back as *Zero Cave* and *Old S.S. Meteor*.
...
- **What lives on a route** — tap a route's name on the Routes tab and the whole encounter table opens: every species it holds, grouped by how you look (grass, surfing, each rod), with the game's own appearance rates, its typing on a badge, and a mark against anything you already own. Under 🎬 Audition this is the menu you are choosing from; under 🌍 Catch 'em All it is the checklist of what you still need. The tick box still cycles **open → caught → burned**, so only the name opens the list. Method Lock (Rule 34) greys out the ways you are not allowed to look.
...
- **Game picker** — choose from all 21 mainline games, **Decay** or **Xenoverse**, or **ROM hack / fan game** to play without a preset battle list. Picking a fan game swaps the species field for that game's own roster, since its Pokémon aren't in the national dex.
...
- **Lineage grid** — all six chairs at a glance, three across on a desktop and one on a phone, so the whole party fits on screen without scrolling. Each occupied chair shows its holder's portrait, level, **nature with its stat effect** (`Adamant +Atk / −SpA`), **ability**, typing and bench depth — everything you'd otherwise open the card to check, on its own line so nothing gets truncated.
- **Catch flow** — species picker, a location dropdown for that game with a **Not a route** group for the origins that are not places (Starter, Gift, In-game trade, Egg, Static encounter — none of which spend an area or appear on the Routes tab, per Rule 19), and an automatic verdict: party or Box. Forgot the location in the rush to roll? The verdict screen offers **📍 Add where you caught it**, and every Pokémon's card has an editable **Caught at** field afterwards, so a blank or a mistake can be fixed at any point. Setting it ticks the Routes tab and, in First Encounter or Audition, is how the tracker knows that area is spent.
...
- **How each Pokémon evolves** — the card lists what it can become and **how**, for the game you are playing. The method is per game, because it genuinely differs: Machoke is a trade in the mainline, a **Switchstone** in Decay and an **Update Mega** in Xenoverse; Jolteon is a Thunder Stone everywhere except Decay, where you raise friendship in a **Voltage area**. Methods that postdate a game are not offered — Red · Blue never mentions friendship, because friendship did not exist — and where a species has more than one, **the regional form is named**: Sandshrew reads *“Level 22 or Ice Stone (Alolan form)”*, Slowpoke *“Trade holding a King’s Rock or Galarica Wreath (Galarian form)”*. A form’s method never appears before the form does, so Sun · Moon shows the Alolan ones and nothing else, and Sword · Shield adds the Galarian and Hisuian.
- **Correcting a tour** — a Pokémon's card has a ✎ beside its remaining majors. It states what the count is; it is **not** a re-roll, and the die's verdict still stands (Rule 31). It is there because every other number in the tracker can be put right, and because a tour that has drifted — through a mistake, or through a bug in an earlier build — otherwise had no way back.
- **Un-ticking a battle you marked by mistake** — tap it again and the wall is un-marked, **and the tour tick it charged is given back** exactly as it was, including a Pokémon that re-enlisted because its lineage had nobody else (Rule 26). A battle that never charged one — outside this run's scope, or partway through a Rule 37 chain — gives nothing back, because it took nothing.
...
- **Tours, tracked for you** — the 🎲 chip on a chair rolls that Pokémon's tour directly, and its card carries the same button until one is rolled. Once the die has spoken there is no re-roll, per Rule 31. Every Pokémon rolls its tour on the die screen the moment it takes a chair, and its card shows the majors it has left (📜 3, 📜 2…). Beat a major and every tour ticks down automatically.
...
- **Next major** — the Overview tab always shows the next wall you haven't beaten, with its Ace Level, and a **✓ Beat it** button so you can tick it off without changing tabs. It runs the same sequence as the Progress list: tours tick, the faint question comes up, the cap moves on. If a handover is owed first (Rules 26–27) the button says **⚠ rotate first** rather than silently doing nothing.
...
**Fan games.** **Pokémon Decay** ships as a selectable game, built from the developers' own public spreadsheets: 55 major battles in story order (8 gyms, the Elite Four, Cynthia, team-boss fights and 5 rival battles) and 94 areas of encounter data. Three corrections to the source: the Lumina City heist is listed **once**, as **Lt. Surge or Koga**, because the game gives you one of the two and never both; four of the sheet's locations were the same place spelled twice — *Kalahri Desert*, *Powerplant*, *Miraged Temple* and a doubled *Route 12 Underwater* — so they are merged into the real ones rather than standing as extra areas worth an extra catch; four more were listed under the map editor's internal label rather than the name the game shows, so *Alph Hollows* is now **Alph Cavern**, *Dragans Cave* is **Silver Cave**, and Greenwood's two halves are **Northern** and **Southern Greenwood**; and **Elder Spenser** is off the list altogether, because he is one of Morty's gym trainers rather than a wall — the sheet lists him among its strong trainers, but his map is the gym, and he stands in it beside Blackbelt Alex and Sage Chow. **Frostvale Gym** is off the *area* list for a related reason: its files do carry a wild table — 100% Glalie, Lv 38–40 — but the tiles that can roll it are all on the floor you fall onto when the ice puzzle breaks under you, and the gym you actually walk through rolls nothing. A pit you drop into by failing a puzzle is not a place you go to spend a catch, and play confirms the Glalie cannot be caught. Decay also brings **34 abilities of its own** — Multiply, Vampirism, Dupleekate and the rest — read from its `abilities_custom.txt`, so the catch flow offers a species its real abilities rather than a blank box. **Every one of those battles has its ace level**, taken from the dev sheet's full team lists — the highest level on the team, not the lead, which is what a cap has to police. Its **encounter tables come from the game's own files**, not the sheet: 65 of its 94 areas carry the real method and the real appearance rate, so walking into Alph Cavern offers Unown and nothing else while the water there holds Magikarp and Wooper. The remaining 29 are event and conditional encounters the sheet lists and the PBS files handle elsewhere; those are availability-only and spin at equal odds, which the tracker says on screen.
...
- **It has a nineteenth type: Sound.** Thirty-two species carry it, and they are not all new ones — Xenoverse re-types mainline Pokémon onto it, so here Ekans and Arbok are Poison/Sound, Jigglypuff and Wigglytuff are Sound/Fairy, and Chatot, Noibat, Litleo and Pyroar all gain it. Bloodline Purity (Rule 33) treats it like any other type, which means a Sound dynasty is a real option.
- **The X Pokémon are the point.** Thirty-two species have an "X" variant from the Xenoverse — a separate catch with its own typing, listed as *Gastly X*, *Carvanha X* and so on. Gastly X is pure **Dragon**, Gengar X is Dragon/Psychic, Electabuzz X is pure **Fire**. Nothing you know about mainline typings helps you, which is exactly why the tracker bundles them.
- **The eighth gym is in Kanto**, in the ruins of the old Vermilion Gym, and the league is the **Four Cardinals** followed by Champion **Aster** — the whole block being one Rule 37 wall. The post-game runs the Cardinals again and finishes with **Versil**, the player's father.

Xenoverse brings **11 abilities of its own** on top of that. Its ordering was taken from the game where the game states it — each gym map records which badge it awards — and confirmed against the community wiki where it doesn't. The resulting ladder climbs without a single dip, from Basil at Lv 12 to Gennaro at Lv 100, which is the check that the order is right rather than merely plausible. Leader names are the English build's: the game's data is Italian, and its own translation file supplies *Ginger*, *Vanilla*, *Nuphar*, *Caspar*, *Henna*, *Abraham* and the rest.
...
Fan games are left alone — their own data is already final for their era, and a ROM hack has no knowable generation, so all of them use the typings they ship with.
...
**Ace level coverage — 465 of 465 major battles.** Every gym, trial, grand trial, titan, Team Star base, noble, villain-team boss, rival battle, Elite Four member and Champion in all 21 mainline games, plus Decay and Xenoverse. There are no gaps and no prompts: pick a game, switch the cap on, and it moves itself from wall to wall for the whole run.
