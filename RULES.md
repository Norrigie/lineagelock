...
- **A cap only ever rises.** Plenty of walls are weaker than the one before them — a rival after a gym, Decay's Jupiter sitting a level under Brawly, Ultra Sun's Hau a full 25 levels under Mina. If the cap dropped to match, a team that was legal five minutes ago would be illegal with no way to fix it, because you cannot un-level a Pokémon. So the cap holds at the highest ace you have faced and waits for the game to catch up. There are **104 such dips across the 22 bundled games**, so this is the normal case, not an edge case.
